        ...
        {currentQuestion.options.map((option, index) => (
          ...
          >
            <div className="flex items-center gap-3 w-full">
              <div className={cn(
                "flex-shrink-0 flex items-center justify-center w-6 h-6 rounded-full border",
                selectedOption === index ? "border-quiz-primary bg-quiz-primary/10" : "border-gray-300 dark:border-gray-600"
              )}>
                {selectedOption === index && <div className="w-3 h-3 rounded-full bg-quiz-primary"></div>}
              </div>
              ...
              {answered && settings.immediateFeedback && (
                <div className="flex-shrink-0">
                  {option.isCorrect ? (
                    <Check className="text-quiz-correct" />
                  ) : (index === selectedOption && <X className="text-quiz-incorrect" />)}
                </div>
              )}
            </div>
      ...
      <div className="mt-8 flex justify-between">
        {!answered ? (
          <Button
            ...
            className="ml-auto quiz-btn quiz-btn-primary"
          >
            Submit Answer
          </Button>
        ) : (
