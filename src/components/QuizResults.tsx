    ...
    <div className="quiz-card animate-fade-in">
      ...
      <div className="divide-y divide-gray-200 dark:divide-gray-700">
        {questions.map((question, qIndex) => {
          ...
          return (
            <div key={qIndex} className="py-6 first:pt-0 last:pb-0">
              <div className="flex gap-3">
                <div className={cn(
                  "flex items-center justify-center h-6 w-6 rounded-full flex-shrink-0 mt-1",
                  isCorrect ? "bg-quiz-correct" : "bg-quiz-incorrect"
                )}>
                  {isCorrect ? <Check className="text-white h-4 w-4" /> : <X className="text-white h-4 w-4" />}
                </div>
                <div>
                      ...
                      >
                        <div className="flex justify-between">
                          ...
                          {(option.isCorrect || oIndex === question.userAnswerIndex) && (
                            <div>
                              {option.isCorrect ? (
                                <Check className="text-quiz-correct h-5 w-5" />
                              ) : (
                                <X className="text-quiz-incorrect h-5 w-5" />
                              )}
                            </div>
                          )}
                        </div>
                        ...
                        {option.explanation && (
                          <div className="quiz-explanation mt-2">
                            {option.explanation}
                          </div>
                        )}
                      </div>
