...
const QuizHistoryDetail: React.FC<QuizHistoryDetailProps> = ({ entry, onBack, onCloseHistory }) => {
  ...
  const formatTimestamp = (timestamp: number): string => {
    ...
    <div className="quiz-card animate-fade-in">
      ...
      {/* Score Summary */}
      <div className="p-3 sm:p-6 bg-gray-50 dark:bg-gray-800 rounded-xl mb-4 sm:mb-8">        <div className="text-center">
          <div className="text-2xl sm:text-4xl font-bold mb-1 sm:mb-2">{score.toFixed(1)}%</div>
      ...
      <h3 className="text-sm sm:text-lg font-bold mb-3 sm:mb-4">Question Details</h3>
      <div className="space-y-3 sm:space-y-6 max-h-[50vh] sm:max-h-[400px] overflow-y-auto scrollbar-webkit">
        {entry.questions.map((question, qIndex) => {
          const selectedOption = question.options[question.userAnswerIndex!];
          const isCorrect = selectedOption?.isCorrect;
          
          return (            <div key={qIndex} className="p-3 sm:p-6 border border-gray-200 dark:border-gray-700 rounded-lg">
              <div className="flex gap-2 sm:gap-4">
                ...
                <div className="flex-grow">
                  <h4 className="text-xs sm:text-base font-medium mb-2 sm:mb-4">
                    {qIndex + 1}. {question.question}
                  </h4>                  <div className="space-y-1.5 sm:space-y-3">
      ...
      <div className="mt-4 sm:mt-8 flex justify-center">
        <Button onClick={handleRetakeQuiz} className="quiz-btn quiz-btn-primary flex items-center gap-2 text-sm sm:text-base">
